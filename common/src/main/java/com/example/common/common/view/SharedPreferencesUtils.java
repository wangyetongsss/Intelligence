...
public class SharedPreferencesUtils {
     ...
     * @param context 上下文
     * @param key     存储的键
     * @param element 所需要移除的元素值
     */
    public static void removeList(Context context, String key, String element) {
        try {
            List sharedPreferences_List = (List) get(context, key);
            if (sharedPreferences_List.size() > 0) {
                for (int i = 0; i < sharedPreferences_List.size(); i++) {
                    if (sharedPreferences_List.get(i).equals(element)) {
                        sharedPreferences_List.remove(i);
                    }
                }
            }
        } catch (Exception e) {
     ...
     * @param context
     */
    public static void clearAppointData(Context context, String key) {
        SharedPreferences sp = getSp(context);
        SharedPreferences.Editor editor = sp.edit();
        editor.remove(key);
        editor.commit();
